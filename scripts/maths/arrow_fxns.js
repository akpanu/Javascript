...
// 2. slope equation
const slope_eqn = (m, x, b) => {
  const gradient = m * x + b
  return gradient
}

// 3. quadratic equation
const quad_eqn = (a, b, c, x) => {
  const y = a * x ** 2 + b * x + c
  return y
}

// 4. quadratic formula
const quad_fmla = (a, b, c) => {
...
// Surface area
// 8. rectangular prism
const sa_rect_prism = (l, w, h) => {
  const sa = 2 * l * w + 2 * l * h + 2 * w * h
  return sa
}

// 9. right prism
const sa_right_prism = (p, h, b) => {
  const sa = p * h + 2 * b
  return sa
}

// 10. cylinder
const sa_cylinder = (r, h) => {
  const sa = 2 * Math.PI * r * h + 2 * Math.PI * r ** 2
  return sa
}

// 11. pyramid
const sa_pyramid = (p, s, b) => {
  const sa = (1 / 2) * p * s + b
  return sa
}
...
const vol_rect_prism = (l, w, h) => {
  const vol = l * w * h
  return vol
}

// 15. right prism
const vol_right_prism = (b, h) => {
  const vol = b * h
  return vol
}

// 16. cylinder
const vol_cylinder = (r, h) => {
  const vol = Math.PI * r ** 2 * h
  return vol
}

// 17. pyramid
const vol_pyramid = (b, h) => {
  const vol = (1 / 3) * b * h
  return vol
}
